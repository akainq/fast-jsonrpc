...
static void *worker_function(void *ptr) {
    ...
    while (1) {
        ...
        /* Execute the job. */
        DEBUG_INFO("get job.",worker)
        job->job_function(job,worker->L);
        DEBUG_INFO("finish job.",worker)
    }
...
int workqueue_init(workqueue_t *workqueue, int numWorkers, const char * root) {
    int i;
    worker_t *worker;
